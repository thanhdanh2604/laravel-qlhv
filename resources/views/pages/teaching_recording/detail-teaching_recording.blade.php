      ...
      <div class="col-sm-12">
        ...
        <div class="card m-b-10">
            <div class="p-20">
                ...
                <span class="f-w-700 text-success f-30">{{$students->full_name}}</span> -
        ...
        @if($teaching_recording->teaching_history!=null)
                @foreach (json_decode($teaching_recording->teaching_history) as $obj_mon_hoc)
                    ...
                    <table id="{{$obj_mon_hoc->ma_giao_vien}}_{{$obj_mon_hoc->ma_mon}}" class="table table-border-horizontal">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>Date</th>
                        ...
                        <tbody>
                            ...
                            @foreach ($obj_mon_hoc->lich_hoc_du_kien as $buoi_hoc)
                                @foreach ($buoi_hoc as $time=> $chi_tiet_gio)
                                    <tr>
                                        <td>  @php
                                            echo $stt; $stt++;
                                            @endphp</td>
                                        <td>
                                            {{date('d/m/Y',$time)}}
                                            @php
                                            if(isset($chi_tiet_gio->starttime)){
                                                echo "<div style=\"color:#1e824c;font-weight:bold\">Start:".$chi_tiet_gio->starttime."</div>";
                                                echo "<span style=\"color:#f22613;font-weight:bold\">End: ".$chi_tiet_gio->endtime."</span>";
                                            }
                                            @endphp
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <select value="" class="form-control" style="display:block" name="day_hours" onchange="ajaxsavehour()">
                                                <option selected value="{{isset($chi_tiet_gio->hours)?$chi_tiet_gio->hours:0}}">{{isset($chi_tiet_gio->hours)?$chi_tiet_gio->hours:0}} Hours</option>
                                            ...
                                            <div data-toggle="collapse" data-target="#panel-tinh-gio-rieng_{{$time}}" aria-expanded="false"  style="margin-top:5px" class="btn btn-outline-secondary">Tính giờ riêng</div>
                                            <div style="width:100%" id="panel-tinh-gio-rieng_{{$time}}" class="tinh-rieng-gio collapse">
                                        ...
                                        <td>
                                            {{isset($chi_tiet_gio->note)?$chi_tiet_gio->note:''}}
                                            <textarea class="form-control" name="" id="" cols="10">@php echo isset($chi_tiet_gio->note)?$chi_tiet_gio->note:''@endphp
                                        </textarea></td>
                                        <td>
                                            <p> Giờ giáo viên:
                                                <span class="text-primary f-w-900">
                                                @php
                                                    if($chi_tiet_gio->dd_prof==1){
                                                    if(isset($chi_tiet_gio->teacher_hours)){
                                                        echo $chi_tiet_gio->teacher_hours;
                                                    }elseif(isset($chi_tiet_gio->hours)){
                                                        echo $chi_tiet_gio->hours;
                                                    }}else{ echo 0;}
                                                @endphp Giờ </span>
                                            </p>
                                            <p> Giờ học sinh:<span class="text-success f-w-900">@php
                                                if($chi_tiet_gio->dd_student==1){
                                                        echo $chi_tiet_gio->hours;
                                                }else{ echo 0;}
                                            @endphp Giờ </span></p>
                                        </td>
                                        <td>
                                            <div data-toggle="tooltip" onclick="xoagiodiemdanh()" class="btn btn-danger">Xóa</div>
                                        </td>
                                    </tr>
                                @endforeach
                            @endforeach
                    ...
                    <div class="modal-body">
                            ...
                            <div id="chosse_teacher">
                                <div class="form-group">
                                    <label style="color:red;font-size:20px">Step 1: Chosse Teacher</label>
